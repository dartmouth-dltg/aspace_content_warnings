...
function setup_offensive_tags(offensive_tags, ext_link, allow_submit) {
  if (offensive_tags.length > 0) {
    var $offensive_content_tags = $('<div class="offensive-tags"></div>');
    apply_offensive_tags(offensive_tags, $offensive_content_tags, ext_link, allow_submit);
  }
}

function setup_iherited_offensive_content_tags(obj, ext_link, allow_submit) {
  var $inherited_tags = $('<div class="offensive-tags inherited-offensive-tags">Applied at the <a href="' + obj.uri + '">' + obj.level + '</a> level:</div>');
  apply_offensive_tags(obj.tags, $inherited_tags, ext_link, allow_submit);
}

function apply_offensive_tags(offensive_tags, tag_wrapper, ext_link, allow_submit) {
  ...
  if (allow_submit) {
    tag_wrapper.append('<button id="offensive-content-sub" class="btn btn-success offensive-content-submit"><i class="fa fa-paper-plane"></i>&nbsp;Help us add a Content Warning</button>');
  }
